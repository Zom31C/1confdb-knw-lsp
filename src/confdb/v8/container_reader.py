...
def decompress_and_extract(src_folder, dest_folder, *, pool=None):
    ...
    bar = progress.start('Разжимаем файлы контейнеров', total)
    try:
        workers = min(8, os.cpu_count() or 1)
        if workers < 2 or len(tasks) < 2:
            for src_folder_, filename, dest_folder_, size in tasks:
                decompress_file_and_extract([src_folder_, filename, dest_folder_])
                bar.update(size, filename)
        else:
            # zlib снимает GIL на inflate, поэтому потоки дают реальную параллельность
            with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as pool:
                futures = {
                    pool.submit(decompress_file_and_extract, [s, f, d]): (f, size)
                    for s, f, d, size in tasks
                }
                for fut in concurrent.futures.as_completed(futures):
                    filename, size = futures[fut]
                    fut.result()
                    bar.update(size, filename)
    finally:
        progress.finish()
